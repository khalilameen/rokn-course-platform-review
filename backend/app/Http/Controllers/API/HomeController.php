...
final class HomeController extends Controller
{
    ...
    public function mainPage(Request $request): JsonResponse
    {
        for ($attempt = 0; $attempt < 2; $attempt++) {
            ...
            $gradeModels = $this->remember(
                'home:grades:v2:' . $catalogRevision,
                300,
                fn () => Grade::query()
                    ->active()
                    ->whereHas(
            ...
            $courses = $this->remember(
                'home:courses:v8:' . $catalogRevision,
                120,
                fn () => $this->duration->attachMany(
            ...
            if ($this->catalogue->revision() === $catalogRevision) {
    ...
    public function mobileMainPage(Request $request): JsonResponse
    {
        for ($attempt = 0; $attempt < 2; $attempt++) {
            ...
            $mainCourses = $this->remember(
                "mobile-home:main-courses:v8:{$catalogRevision}",
                120,
                fn () => $this->duration->attachMany(
                    $this->catalogue->orderForDiscovery(
                        $this->catalogue->applyPublicContract(Course::query())
            ...
            $hasHomeRowControls = DatabaseCapabilities::hasColumn('classifications', 'show_on_home')
                && DatabaseCapabilities::hasColumn('classifications', 'home_order');
            $hasCourseHomeOrder = DatabaseCapabilities::hasColumn('courses', 'home_sort_order');
            ...
            foreach ($classifications as $classification) {
                ...
                $courses = $this->remember(
                    "mobile-home:classification:{$classification->id}:courses:v11:{$catalogRevision}:" . ($hasCourseHomeOrder ? 'managed' : 'legacy'),
                    120,
                    fn () => $this->duration->attachMany(
                        $this->catalogue->orderForDiscovery(
    ...
    private function catalogueChanged(): JsonResponse
    {
        return response()->json([
            'status' => 409,
            'success' => false,
            'code' => 'catalogue_changed',
            'message' => "تغيّرت قائمة الكورسات\nنحدّثها الآن",
            'data' => [
