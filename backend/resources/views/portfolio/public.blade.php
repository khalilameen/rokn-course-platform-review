...
<main class="wrap">
    ...
    <section><div class="section-head"><h2>الشهادات</h2></div><div class="grid">@foreach($portfolio['certificates'] as $certificate)<div class="certificate {{ ($portfolio['highlighted_certificate']['public_id'] ?? null) === $certificate['public_id'] ? 'highlight' : '' }}"><div class="verified">{{ $certificate['status'] !== 'active' ? 'شهادة ملغاة' : ((($certificate['verification_level'] ?? 'completion') === 'reviewed_project') ? 'إتمام ومشروع بمراجعة بشرية ✓' : 'إتمام الكورس موثق ✓') }}</div><h3>{{ $certificate['course_name'] ?? $certificate['course']['name'] ?? 'شهادة إتمام' }}</h3>@if(!empty($certificate['certificate_text']))<div class="muted">{{ $certificate['certificate_text'] }}</div>@endif<div class="muted">{{ $certificate['generated_at'] }}</div></div>@endforeach</div></section>
    @endif
</main>
