        ...
        <form action="{{ route('admin.coin-earning-methods.update', $coinEarningMethod->id) }}" method="POST">
            ...
            <div class="row mt-2">
                <div class="col-md-8 mb-3">
                    ...
                    <small class="form-text text-muted">اتركه فارغًا لمهام السوشيال كي تستخدم رابط الحساب المركزي من إعدادات التصميم.</small>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="text" name="campaign_key" class="form-control @error('campaign_key') is-invalid @enderror" value="{{ old('campaign_key', $coinEarningMethod->campaign_key) }}">
                    @error('campaign_key')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">بداية الحملة</label>
                    <input type="datetime-local" name="starts_at" class="form-control" value="{{ old('starts_at', \App\Support\BusinessClock::forDateTimeInput($coinEarningMethod->starts_at)) }}">
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">نهاية الحملة</label>
                    <input type="datetime-local" name="ends_at" class="form-control" value="{{ old('ends_at', \App\Support\BusinessClock::forDateTimeInput($coinEarningMethod->ends_at)) }}">
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">إجمالي المطالبات المتاحة</label>
                    <input type="number" min="1" name="total_claim_limit" class="form-control" value="{{ old('total_claim_limit', $coinEarningMethod->total_claim_limit) }}" placeholder="بلا سقف">
                </div>
            </div>
