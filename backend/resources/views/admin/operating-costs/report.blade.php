...
<div class="admin-page animated fadeIn">
    ...
    <div class="statistics-grid">
        ...
        <div class="stat-card"><span class="stat-counter">{{ number_format($report['active_enrollments']) }}</span><span class="stat-label">اشتراكات نشطة</span></div>
    ...
    <div class="row mt-4">
        <div class="col-xl-6 mb-4"><div class="card admin-card h-100"><div class="card-header"><strong>حسب الفئة السعرية</strong></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>الفئة</th><th>طلاب</th><th>صافي/طالب</th><th>تكلفة/طالب</th><th>التكلفة من الصافي</th><th>الهامش</th></tr></thead><tbody>@forelse($report['plan_breakdown'] as $name => $row)<tr><td>{{ $name }}</td><td>{{ number_format($row['students']) }}</td><td>{{ $row['average_net_per_student_egp'] === null ? '—' : number_format($row['average_net_per_student_egp'], 2) }}</td><td>{{ $row['average_cost_per_student_egp'] === null ? '—' : number_format($row['average_cost_per_student_egp'], 2) }}</td><td>{{ $row['cost_to_net_revenue_percentage'] === null ? '—' : number_format($row['cost_to_net_revenue_percentage'], 2).'%' }}</td><td>{{ $row['margin_egp'] === null ? '—' : number_format($row['margin_egp'], 2).' ج.م' }}</td></tr>@empty<tr><td colspan="6" class="text-center text-muted">لا توجد بيانات.</td></tr>@endforelse</tbody></table></div></div></div>
        <div class="col-xl-6 mb-4"><div class="card admin-card h-100"><div class="card-header"><strong>حسب الكورس</strong></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>الكورس</th><th>طلاب</th><th>الصافي</th><th>التكلفة</th><th>النسبة</th><th></th></tr></thead><tbody>@forelse($report['course_breakdown'] as $row)<tr><td>{{ $row['course_name'] }}</td><td>{{ number_format($row['students']) }}</td><td>{{ $row['net_egp'] === null ? '—' : number_format($row['net_egp'], 2) }}</td><td>{{ $row['service_cost_egp'] === null ? '—' : number_format($row['service_cost_egp'], 2) }}</td><td>{{ $row['cost_to_net_revenue_percentage'] === null ? '—' : number_format($row['cost_to_net_revenue_percentage'], 2).'%' }}</td><td><a href="{{ route('admin.courses.show', $row['course_id']) }}#commercial-report">التفاصيل</a></td></tr>@empty<tr><td colspan="6" class="text-center text-muted">لا توجد بيانات.</td></tr>@endforelse</tbody></table></div></div></div>
    </div>

    <div class="card admin-card mb-4"><div class="card-header"><strong>حسب مصدر الإتاحة</strong></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>المصدر</th><th>الاشتراكات</th><th>الصافي</th><th>التكلفة</th><th>الهامش</th><th>التكلفة من الصافي</th></tr></thead><tbody>@forelse($report['source_breakdown'] as $name => $row)<tr><td>{{ $name }}</td><td>{{ number_format($row['students']) }}</td><td>{{ $row['net_egp'] === null ? '—' : number_format($row['net_egp'], 2).' ج.م' }}</td><td>{{ $row['service_cost_egp'] === null ? '—' : number_format($row['service_cost_egp'], 2).' ج.م' }}</td><td>{{ $row['margin_egp'] === null ? '—' : number_format($row['margin_egp'], 2).' ج.م' }}</td><td>{{ $row['cost_to_net_revenue_percentage'] === null ? '—' : number_format($row['cost_to_net_revenue_percentage'], 2).'%' }}</td></tr>@empty<tr><td colspan="6" class="text-center text-muted">لا توجد بيانات.</td></tr>@endforelse</tbody></table></div></div>
