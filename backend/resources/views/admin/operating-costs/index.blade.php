...
<div class="admin-page animated fadeIn">
    ...
    <div class="card admin-card mt-4"><div class="card-header"><strong>تكلفة وربحية الطلاب حسب الكورس</strong></div><div class="card-body"><p class="text-muted">داخل تقرير كل كورس ستجد الاستهلاك والتكلفة الفعلية والهامش لكل طالب ولكل فئة سعرية، مع تصدير CSV.</p><div class="table-responsive"><table class="table table-hover mb-0"><thead><tr><th>الكورس</th><th>الطلاب النشطون</th><th></th></tr></thead><tbody>@forelse($courses as $course)<tr><td>{{ $course->name_ar }}</td><td>{{ number_format($course->active_enrollments_count) }}</td><td><a class="btn btn-sm btn-outline-primary" href="{{ route('admin.courses.show', $course) }}#commercial-report">فتح تقرير الربحية</a></td></tr>@empty<tr><td colspan="3" class="text-center text-muted">لا توجد كورسات.</td></tr>@endforelse</tbody></table></div></div></div>
</div>
@endsection
