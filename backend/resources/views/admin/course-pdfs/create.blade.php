...
@section('page.title', 'إضافة ملف PDF')

@section('styles')
@include('admin.course-pdfs.partials._dynamic_styles')
<link rel="stylesheet" href="{{ asset('admin/assets/css/course-pdfs-create.css') }}">
@endsection
        ...
        <form action="{{ route('admin.courses.pdfs.store', $course) }}" method="POST" enctype="multipart/form-data" id="coursePdfForm">
            ...
            <div class="form-group">
                ...
                <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" 
                       value="{{ old('title') }}" placeholder="أدخل عنوان الملف" required>
                @error('title')
            ...
            <div class="form-group">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                          rows="3" placeholder="أدخل وصف الملف (اختياري)">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="title_en" class="form-control @error('title_en') is-invalid @enderror" 
                       value="{{ old('title_en') }}" placeholder="Enter file title in English">
                @error('title_en')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="description_en" class="form-control @error('description_en') is-invalid @enderror" 
                          rows="3" placeholder="Enter file description in English">{{ old('description_en') }}</textarea>
            ...
            <div class="form-group">
                <label class="form-label">
                    اختر ملف PDF <span class="required">*</span>
                </label>
                <div class="file-upload-area" id="dropZone">
                    <div class="file-upload-icon">
                        <i class="fa fa-cloud-upload"></i>
                    </div>
                    <p class="file-upload-text">اسحب الملف هنا أو انقر للاختيار</p>
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="order" class="form-control @error('order') is-invalid @enderror" 
                                   value="{{ old('order', $maxOrder + 1) }}" min="0">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <div class="toggle-switch-container mt-2">
                                ...
                                <span class="toggle-status active" id="statusBadge">مفعّل</span>
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isActiveCheckbox && statusBadge) {
        ...
        updateStatusBadge(); // Initial state
    }
});
</script>
