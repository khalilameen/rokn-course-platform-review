...
export const StackedToastProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const dismissToastByKey = useCallback(
    (key: string) => {
      setStackedToasts(prev => {
        const toastToDismiss = prev.find(item => item.key === key);
        if (!toastToDismiss) return prev;
  ...
  }, [clearAllStackedToasts, showStackedToast, dismissToastByKey]);
