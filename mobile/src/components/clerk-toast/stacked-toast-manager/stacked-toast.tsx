...
const StackedToast: React.FC<StackedToastProps> = ({
  ...
  useEffect(() => {
    if (stackedSheet.duration && stackedSheet.duration > 0 && isActiveStackedToast) {
      const timer = setTimeout(() => {
        // Trigger dismiss animation using runOnUI to ensure it runs on UI thread
        translateX.value = withSpring(
          -windowWidth,
          {
            dampingRatio: 1,
      ...
      }, stackedSheet.duration);

      return () => clearTimeout(timer);
    }
  }, [stackedSheet.duration, isActiveStackedToast, stackedToastId, onDismiss, translateX, windowWidth]);
