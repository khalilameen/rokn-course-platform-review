...
test('Android release snapshot covers the resolved closure and ships exact texts', () => {
  ...
  assert.equal(snapshot.dependencyCount, 197);
  assert.equal(snapshot.dependencies.length, 197);
  assert.equal(
    snapshot.dependencies.filter(item => item.artifacts.length === 0).length,
    20,
  );
  assert.equal(snapshot.projectComponentCount, 18);
  assert.equal(snapshot.projectComponents.length, 18);
...
test('native gate fails closed while the checked-in Podfile.lock is stale', () => {
  const result = childProcess.spawnSync(
    process.execPath,
    [path.join(ROOT, 'scripts', 'generate-native-third-party-notices.js'), '--check'],
    {cwd: ROOT, encoding: 'utf8'},
  );
  assert.notEqual(result.status, 0);
  const output = `${result.stdout}\n${result.stderr}`;
  assert.match(output, /CocoaPods notices cannot be generated from a stale lock/);
  assert.match(output, /React 0\.83\.1, but react-native is 0\.83\.10/);
  assert.match(output, /missing autolinked pods/);
  assert.match(
    output,
    /Gemfile\.lock resolves CocoaPods 1\.15\.2, but ios\/Podfile\.lock was generated by 1\.16\.2/,
  );
  assert.match(output, /maintained Ruby >=3\.2\.0; found 2\.6\.10/);
  assert.match(output, /Bundler >=2\.5\.0; found 1\.17\.2/);
  assert.match(output, /bundle exec pod install/);
  ...
  const blockedArtifact = fs.readFileSync(
    path.join(ROOT, 'ios', 'Rokn', 'NATIVE_THIRD_PARTY_NOTICES.md'),
    'utf8',
  );
  assert.doesNotMatch(
    blockedArtifact,
